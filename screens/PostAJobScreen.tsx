      ...
      <main className="pt-24 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
            <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg border border-gray-200 p-8 md:p-12">
              <h2 className="text-3xl font-bold text-amo-dark mb-8">Submit Job Details</h2>
              <form className="space-y-8" onSubmit={e => e.preventDefault()}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label htmlFor="job-title" className="block text-sm font-medium text-gray-700 mb-1">Job Title</label>
                    <input type="text" id="job-title" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amo-orange" placeholder="e.g., Senior AI Engineer" />
                  </div>
                  <div>
                    <label htmlFor="company-name" className="block text-sm font-medium text-gray-700 mb-1">Company Name</label>
                    <input type="text" id="company-name" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amo-orange" placeholder="Your Company Inc." />
                  </div>
                </div>
                <div>
                  <label htmlFor="job-description" className="block text-sm font-medium text-gray-700 mb-1">Job Description</label>
                  <textarea id="job-description" rows={6} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amo-orange" placeholder="Describe the role, responsibilities, and qualifications..."></textarea>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label htmlFor="location" className="block text-sm font-medium text-gray-700 mb-1">Location</label>
                    <input type="text" id="location" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amo-orange" placeholder="e.g., San Francisco / Remote" />
                  </div>
                  <div>
                    <label htmlFor="employment-type" className="block text-sm font-medium text-gray-700 mb-1">Employment Type</label>
                    <select id="employment-type" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amo-orange bg-white">
                ...
                <div>
                  ...
                  <input type="text" id="salary-range" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amo-orange" placeholder="e.g., $120,000 - $160,000 USD" />
                </div>
                <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 p-2 border border-gray-300 rounded-lg">
                        {skills.map(skill => (
                            <span key={skill} className="flex items-center gap-1.5 bg-orange-100 text-amo-orange text-sm font-semibold px-2 py-1 rounded">
                                {skill}
                                <button onClick={() => removeSkill(skill)} className="font-bold text-amo-orange hover:text-orange-700">×</button>
                            </span>
                        ))}
                        <input
                            type="text"
                ...
                <div>
                    ...
                    <input type="text" id="application-link" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amo-orange" placeholder="https://yourcompany.com/apply or jobs@company.com" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Upload Company Logo (Optional)</label>
                    <div 
                      className={`w-full p-6 border-2 border-dashed rounded-lg text-center cursor-pointer transition-colors ${isDragging ? 'border-amo-orange bg-orange-50' : 'border-gray-300 bg-gray-50 hover:border-gray-400'}`}
                ...
                <div className="pt-6 border-t border-gray-200 flex items-center gap-4">
                    <button type="submit" className="bg-amo-orange text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-opacity-90 transition-all">Publish Job</button>
            ...
            <aside className="lg:col-span-1">
              <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 sticky top-24">
                <h2 className="text-2xl font-bold text-amo-dark mb-6">How It Works</h2>
                <div className="space-y-6">
                  {sidebarSteps.map((step, index) => (
                    <div key={index} className="flex items-start gap-4">
                      <div className="flex-shrink-0 w-10 h-10 bg-orange-100 text-amo-orange rounded-full flex items-center justify-center">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16">
           <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 flex flex-col md:flex-row justify-between items-center text-center md:text-left">
                <div>
                    <h2 className="text-2xl font-bold text-amo-dark">Need help attracting talent?</h2>
                    <p className="text-gray-600 mt-1">Our team can help you craft the perfect job description and promote it to our community.</p>
                </div>
                <button className="mt-4 md:mt-0 flex-shrink-0 bg-amo-dark text-white font-bold py-3 px-6 rounded-lg shadow-md hover:bg-black transition-all flex items-center gap-2">
