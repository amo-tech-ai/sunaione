...
const WizardSteps: React.FC<WizardStepsProps> = ({ deckData, setDeckData, onFinish }) => {
  ...
  const renderRefineButton = (fieldName: keyof DeckData, label: string) => (
    <button
      ...
      className="absolute top-0 right-0 mt-1 mr-1 text-xs bg-orange-100 text-amo-orange font-semibold py-1 px-2 rounded-md hover:bg-orange-200 transition-colors flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed"
    >
    ...
    return (
      <WizardScreen title="The Problem" subtitle="Clearly define the problem you're solving. What is the pain point for your customers?" currentStep={currentStepNumber} totalSteps={totalSteps} onNext={nextStep} onBack={prevStep} canGoNext={!!deckData.problem && !!deckData.solution}>
        ...
        <div className="relative mt-6">
          ...
          <textarea name="solution" id="solution" rows={4} value={deckData.solution} onChange={handleInputChange} className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-amo-orange focus:border-transparent transition" placeholder="Briefly explain your product or service and how it addresses the problem."></textarea>
    ...
    return (
      <WizardScreen title="Market & Model" subtitle="Who are your customers and how will you make money?" currentStep={currentStepNumber} totalSteps={totalSteps} onNext={nextStep} onBack={prevStep} canGoNext={!!deckData.targetAudience && !!deckData.businessModel}>
        ...
        <div className="relative mt-6">
          ...
          <textarea name="businessModel" id="businessModel" rows={4} value={deckData.businessModel} onChange={handleInputChange} className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-amo-orange focus:border-transparent transition" placeholder="e.g., 'SaaS model with monthly subscriptions', 'A marketplace taking a 10% commission on each transaction.'"></textarea>
    ...
    return (
      <WizardScreen title="Traction & Team" subtitle="Show what you've achieved and who is behind the vision." currentStep={currentStepNumber} totalSteps={totalSteps} onNext={nextStep} onBack={prevStep} canGoNext={!!deckData.traction && !!deckData.teamMembers}>
        ...
        <div className="relative mt-6">
          ...
          <textarea name="teamMembers" id="teamMembers" rows={4} value={deckData.teamMembers} onChange={handleInputChange} className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-amo-orange focus:border-transparent transition" placeholder="List the founders and key members with their relevant experience. E.g., 'Jane Doe (CEO) - 10 years at Google. John Smith (CTO) - Lead engineer at Stripe.'"></textarea>
    ...
    return (
      <WizardScreen title="The Ask" subtitle="How much are you raising and what will you use it for?" currentStep={currentStepNumber} totalSteps={totalSteps} onNext={nextStep} onBack={prevStep} canGoNext={!!deckData.fundingAmount && !!deckData.useOfFunds} isLastStep>
        ...
        <div className="relative mt-6">
          ...
          <textarea name="useOfFunds" id="useOfFunds" rows={4} value={deckData.useOfFunds} onChange={handleInputChange} className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-amo-orange focus:border-transparent transition" placeholder="Break it down by category. E.g., '40% for product development, 30% for marketing and sales, 20% for new hires, 10% for operational costs.'"></textarea>
        ...
        <div className="mt-6">
            ...
            <select name="template" id="template" value={deckData.template} onChange={handleInputChange} className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-amo-orange focus:border-transparent transition bg-white">
