...
const DetailCard: React.FC<{ title: string; children: React.ReactNode; className?: string }> = ({ title, children, className = '' }) => (
    <div className={`bg-white rounded-2xl shadow-md border border-gray-200 p-8 ${className}`}>
        <h2 className="text-2xl font-bold text-sunai-dark mb-6">{title}</h2>
        {children}
    </div>
);
...
const BenefitCard: React.FC<{ icon: React.ElementType; title: string; description: string }> = ({ icon: Icon, title, description }) => (
    <div className="bg-gray-50/50 p-6 rounded-xl text-center transition-all hover:shadow-lg hover:-translate-y-1 border border-gray-200">
        <div className="bg-orange-100 text-sunai-orange w-12 h-12 rounded-full flex items-center justify-center mb-4 mx-auto">
            <Icon className="w-6 h-6" />
        </div>
        <h3 className="text-lg font-bold text-sunai-dark mb-1">{title}</h3>
...
const StepCard: React.FC<{ step: number; title: string; description: string }> = ({ step, title, description }) => (
    <div className="flex items-start gap-4">
        <div className="flex-shrink-0 w-10 h-10 bg-sunai-orange text-white font-bold text-lg rounded-full flex items-center justify-center">
            {step}
        </div>
        <div>
...
const PerkDetailScreen: React.FC<PerkDetailScreenProps> = ({ perkId, allPerks, setCurrentScreen, onViewDetails }) => {
    ...
    if (!perk) {
        return (
            <div>
                <button onClick={() => setCurrentScreen(Screen.Perks)} className="flex items-center gap-2 text-gray-600 font-semibold mb-6 hover:text-sunai-dark transition-colors">
    ...
    return (
        <div className="max-w-4xl mx-auto">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <DetailCard title="How to Redeem This Offer">
                        <div className="space-y-6">
                            <StepCard step={1} title="Sign up or log in" description="Create your free Sun AI account in seconds to get started." />
                    ...
                    {/* Related Perks */}
                    <div>
                        <h2 className="text-2xl font-bold text-sunai-dark mb-4">More Perks You Might Like</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            {relatedPerks.map(p => (
                                <div key={p.id} className="bg-white p-4 rounded-xl border border-gray-200 text-center transition-all hover:shadow-lg hover:-translate-y-1">
                                    ...
                                    <p className="font-bold text-sunai-orange text-sm">{p.offer}</p>
                ...
                <aside className="lg:col-span-1 sticky top-8">
                     <div className="bg-white rounded-2xl shadow-md border border-gray-200 p-6 space-y-3">
                         <button 
                             ...
                             className="w-full bg-sunai-orange text-white font-bold py-3 px-6 rounded-lg shadow-md hover:bg-opacity-90 transition-all text-base"
                         >
                             Claim This Perk
                         </button>
                          <button 
            ...
            <div className="mt-16 text-center bg-gradient-to-r from-orange-100 to-amber-100 p-12 rounded-2xl">
                 <h2 className="text-3xl font-bold text-sunai-dark">Ready to unlock all founder perks?</h2>
                 <p className="text-lg text-gray-700 mt-2 mb-6">Join hundreds of startups saving thousands with Sun AI.</p>
                 <button 
                     onClick={() => setCurrentScreen(Screen.Welcome)}
                     className="bg-sunai-dark text-white font-bold py-3 px-8 rounded-lg shadow-lg hover:bg-black transition-all"
                 >
